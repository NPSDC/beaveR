...
#' the geometric mean sequencing depth is computed
#' @param quiet display no messages
#' @param sf matrix of size factors computed for each inf replicate (default NULL)
...
scaleInfReps <- function(tse, sf = NULL, lengthCorrect = TRUE,
    ...
    if (is.null(meanDepth) & is.null(sf)) {
    ...
    if(is.null(sf)) message("Setting inf rep depth to mean depth")
    for (k in seq_len(nreps)) {
        if (!quiet) svMisc::progress(k, max.value=nreps, init=(k==1), gui=FALSE)
        if (lengthCorrect) {
            length <- length / exp(rowMeans(log(length)))
